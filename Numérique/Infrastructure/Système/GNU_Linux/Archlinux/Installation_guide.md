...
cryptsetup \
    ...
    cryptsetup luksDump /dev/disk/by-partlabel/akpsystem-sys-raid0-1-encrypt && \
    cryptsetup luksDump /dev/disk/by-partlabel/akpsystem-sys-raid0-2-encrypt && \
    cryptsetup luksDump /dev/disk/by-partlabel/akpsystem-swap-1-encrypt && \
    cryptsetup luksDump /dev/disk/by-partlabel/akpsystem-swap-2-encrypt
...
mkfs.btrfs \
    ...
    mount \
        ...
        LABEL=akpsystem \
        /mnt/.snapshots && \
    mount -o noatime /dev/disk/by-partlabel/EFI /mnt/boot && \
    lsblk
```
...
akpsystem-sys-raid0-2 LABEL=akpsystem-sys-raid0-2-encrypt none timeout=360,discard,no-read-workqueue,no-write-workqueue
akpsystem-swap-1 LABEL=akpsystem-swap-1-encrypt none timeout=360,discard,no-read-workqueue,no-write-workqueue
akpsystem-swap-2 LABEL=akpsystem-swap-2-encrypt none timeout=360,discard,no-read-workqueue,no-write-workqueue
...
refind-install --alldrivers --usedefault $DEVICE_BOOT && \
...
sbctl sign -s /boot/EFI/BOOT/drivers_x64/btrfs_x64.efi && \
sbctl sign -s  && \
sbctl sign -s  && \
sbctl sign -s  && \
sbctl sign -s  && \
